...
class SimpletronWithReLU(Gladiator):
    ...
    def predict(self, credit_score: float) -> int:
        # Apply ReLU to the linear combination of input and weight
        raw_output = credit_score * self.weight
        relu_output = max(0, raw_output)  # ReLU activation

        # Apply step function to get binary output
        return 1 if relu_output >= 0.5 else 0
