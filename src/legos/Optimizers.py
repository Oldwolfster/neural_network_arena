...
standard_gbs_headers_single         = ["Input", "Blame", "Raw Adj"]
...
class Optimizer:
    ...
    def _intercept_update(self,neuron, input_vector, accepted_blame, t, config, epoch, iteration, batch_id):
        raw_logs = sgd_update(neuron, input_vector, accepted_blame, t, config)
        ctx = [
            epoch ,             #epoch, iteration,  neuron.nid,  batch_id,
            iteration,
            neuron.nid,
            batch_id
        ]

        # Combine context with result of update for final logs to return
        final_logs = []
        for row in raw_logs:
            weight_id, *rest = row
...
def sgd_updateOrig(neuron, input_vector, accepted_blame, t, config, epoch, iteration, batch_id):
    """
    Full Batch SGD — accumulate gradients during backprop,
    apply the final adjustment once per batch using average blame.
    This method is called **during each sample**, to accumulate blame.
...
OptimizerOrig_SGD = OptimizerOrig(
...
OptimizerOrig_Adam = OptimizerOrig(
    name = "Adam",
