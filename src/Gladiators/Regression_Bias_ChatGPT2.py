...
class Regression_Bias_ChatGPT2(Gladiator):
    ...
    def training_iteration(self, training_data) -> GladiatorOutput:
        ...
        prediction = (input * self.weight) + self.bias    # Step 1: Prediction
        error = result - prediction  # Step 2: Calculate error (MSE can be applied here later, starting with simple error)
        new_weight = self.learning_rate * error * input
    ...
    def apredict(self, credit_score: float) -> float:
        """
