...
class Salary2InputsLinear(BaseArena):
    ...
    def generate_training_data(self) -> List[Tuple[float, float, float, float]]:
        ...
        for _ in range(self.num_samples):
            ...
            # NOTE: COMES BEFORE INTERATION - Proving INTERACTION is not part of salary calc
            ...
            # Scale features
            #years_experience /= 40  # Now in [0, 1]
            #college /= 8            # Now in [0, 1]
            #interaction /= 320      # Max interaction value
            # Scale salary (assuming max_salary is known or estimated)
            # Lets try with and without scaling the target
            # salary /= max_salary
