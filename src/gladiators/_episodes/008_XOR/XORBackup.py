...
class SuzukiHayabusa_XOR(Gladiator):
    ...
    def training_iteration(self, training_data) -> float:
        ...
        prediction = self.forward_pass(inp_0, inp_1)
...
Copy code
        ...
        """
    def forward_pass(self,inp_0,inp_1) -> float:
        """
        :param inputs from I1 and I2
        :return prediction
        Hello MLP world!
        """
        print (f"STARTING Iteration FORWARD PASS*****************\tinput_0={inp_0}\tinput_1={inp_1}\t")
        # Step 1: Compute the output of the first hidden neuron
        h0_raw          = ( inp_0 * self.neurons[0].weights[0] +
                            inp_1 * self.neurons[0].weights[1] +
                            self.neurons[0].bias)

        print(f"h0_raw= ====================( {inp_0} * {self.neurons[0].weights[0]} +{inp_1} * {self.neurons[0].weights[1]} +{self.neurons[0].bias})")

        h0_output   =       self.tanh(h0_raw)
        print (f"H0 Complete:\th0_raw={h0_raw}\th0_output={h0_output}")

        # Step 2: Compute the output of the second hidden neuron
        h1_raw          = ( inp_0 * self.neurons[1].weights[0] +
                            inp_1 * self.neurons[1].weights[1] +
                            self.neurons[1].bias)
        h1_output       = self.tanh(h1_raw)
        print (f"H1 Complete:\th1_raw={h1_raw}\th1_output={h1_output}")

        # Step 3: Compute the output of the output neuron - that is the predicton
        out_raw         = ( h0_output * self.neurons[2].weights[0] +
                            h1_output * self.neurons[2].weights[1] +
                            self.neurons[2].bias)
        out_tanhed      =   self.tanh(out_raw)              # Apply tahn function
        prediction      =   1 if out_tanhed > 0 else 0      # Apply step function
        print (f"FORWARD PASS complete *****************\tout_raw={out_raw}\tout_tanhed={out_tanhed}\tprediction={prediction}")
