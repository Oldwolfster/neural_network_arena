...
class NeuralNetwork:
    ...
    def fit(self, X, y, epochs=1000, displayUpdate = 10):
        ...
        for epoch in np.arange(0, epochs):  #loop over epochs
            for (x, target) in zip (X, y):      #loop through samples
                #if epoch < 1000000:                    print(f"epoch {epoch}\t inputs{x}")
    ...
    def fit_partial(self, x, y):
        ...
        for layer in np.arange(0, len(self.W)):
            ...
            # >>> ADD THIS LINE so subsequent layers match the (n+1) bias rows:
            if layer < len(self.W) - 1:
                out = np.c_[out, np.ones((out.shape[0], 1))]
    ...
    def predict(self, X, addBias=True):
        ...
        # if requested, add a column of 1’s to the input
        if addBias:
        ...
        for layer in np.arange(0, len(self.W)):
            ...
            # if this is NOT the last layer, append a bias column
            if layer < len(self.W) - 1:
                p = np.c_[p, np.ones((p.shape[0], 1))]

        # p is now your output
        return p

    def predictwrong(self, X, addBias=True):
