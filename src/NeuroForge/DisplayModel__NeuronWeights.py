...
from src.engine.Utils import draw_rect_with_border, draw_text_with_background, ez_debug, check_label_collision
...
class DisplayModel__NeuronWeights:
    ...
    def __init__(self, neuron, ez_printer):
        ...
        self.padding_top                = 3
        self.gap_between_weight_bars    = 0
    ...
    def initialize(self):
        ...
        if self.num_weights > 0:
            self.bar_height= self.calculate_bar_height(num_weights=self.num_weights, neuron_height=self.neuron_height, padding_top=self.padding_top,padding_bottom=self.padding_bottom, gap_between_weight_bars= self.gap_between_weight_bars,gap_between_weights=self.gap_between_weights)
        print(f"Bar height= {self.bar_height}")
        #ez_debug(height=self.bar_height,num=self.num_weights)
    ...
    def calculate_bar_height(self, num_weights, neuron_height, padding_top, padding_bottom, gap_between_weight_bars, gap_between_weights):
        ...
        ez_debug(aval=available_height, neuron_height=self.neuron.location_height, pt=padding_top, pb=padding_bottom, bh=self.BANNER_HEIGHT,total_gaps=total_gaps)
    ...
    def draw_two_bars_for_one_weight(self, screen, x, y, width_self, width_global, bar_height, bar_gap, weight_value, weight_id):
        ...
        label_rects=[]
        label_space = self.draw_weight_label(screen, label_text_global, global_rect, (255, 165, 0))  # Label for global bar
        label_rects.append(label_space)
        label_space = self.draw_weight_label(screen, label_text_local, self_rect, (0, 128, 0))  # Label for self bar
        label_rects.append(label_space)
        self.draw_weight_index_label(screen, weight_id, y+self.bar_height-9, label_rects)
    ...
    def draw_weight_index_label(self, screen, weight_index, y_pos,existing_labels_rects):
        """
        ...
        #Get rect for index label.
        font = pygame.font.SysFont(None, self.font_size_weight)
        text_surface = font.render(label_text, True, (0,0,0)) #color does not matter.
        text_rect = text_surface.get_rect()
        text_rect.topleft = label_x,label_y

        if self.neuron.model_id=="NeuroForge_Experiment" and self.neuron.nid==0:
            ez_debug(new=text_rect, old=existing_labels_rects)

        if self.neuron.layer == 0 and self.neuron.location_left > text_rect.width + 5:
            label_x = self.neuron.location_left - text_rect.width - 5
