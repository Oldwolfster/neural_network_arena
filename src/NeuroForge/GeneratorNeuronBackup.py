...
class GeneratorNeuron:
    ...
    def create_neurons(the_model, max_act: float, margin=20, gap=30, max_neuron_size=350):
        ...
        input_scaler_offset = 0 if GeneratorNeuron.model.config.scaler.inputs_are_unscaled else 1
        prediction_scaler_offset = 0 if GeneratorNeuron.model.config.scaler.target_is_unscaled else 1
        scaler_offset2 = input_scaler_offset + prediction_scaler_offset
        ...
        max_layers += input_scaler_offset + scaler_offset2
        ...
        print(f"x_positions={x_positions}\t size = {size}")
        # 🔹 Create neurons
        ...
        for layer_index, neuron_count in enumerate(GeneratorNeuron.model.config.architecture):
            ...
            x_coord = x_positions[layer_index + input_scaler_offset]
    ...
    def calculate_neuron_size( margin, gap, max_neuron_size):
        ...
        max_layers = len(padded_architecture)  # Total number of layers

        if GeneratorNeuron.model.config.scaler.target_is_unscaled:
            max_layers += 1
